...
const loadArticleDetail = async () => {
  try {
    loading.value = true
    // 调用API获取文章详情
    const response = await knowledgeApi.getArticle(Number(articleId))
    ...
    ElMessage.error((error as Error).message || '获取文章详情失败')
  } finally {
    loading.value = false
  }
}
...
const submitForm = async () => {
  ...
  try {
    ...
    // 调用API更新文章
    await knowledgeApi.updateArticle(Number(articleId), articleForm)
    
    ElMessage.success('保存成功')
    loading.value = false
    // 跳转到文章详情页面
    router.push(`/knowledge/detail/${articleId}`)
  } catch (error) {
